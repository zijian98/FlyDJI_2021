...
public class PlaybackActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    File destDir = new File(Environment.getExternalStorageDirectory().getPath() + "/MediaManager/");
    ...
    protected void onDestroy() {
        ...
        if (isMavicAir2() || isM300()) {
            if (FlyDJI.getCameraInstance() != null) {
                FlyDJI.getCameraInstance().exitPlayback(djiError -> {
                    if (djiError != null) {
                        FlyDJI.getCameraInstance().setFlatMode(SettingsDefinitions.FlatCameraMode.PHOTO_SINGLE, djiError1 -> {
                            if (djiError1 != null){
                ...
                FlyDJI.getCameraInstance().setMode(SettingsDefinitions.CameraMode.SHOOT_PHOTO, djiError -> {
                    if (djiError != null){
    ...
    void initUI() {
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(PlaybackActivity.this, RecyclerView.VERTICAL,false);
    ...
    private void initMediaManager() {
        if (FlyDJI.getProductInstance() == null) {
            mediaFileList.clear();
            mListAdapter.notifyDataSetChanged();
            DJILog.e(TAG, "Product disconnected");
            return;
        } else {
            if (null != FlyDJI.getCameraInstance() && FlyDJI.getCameraInstance().isMediaDownloadModeSupported()) {
                mMediaManager = FlyDJI.getCameraInstance().getMediaManager();
                if (null != mMediaManager) {
                    mMediaManager.addUpdateFileListStateListener(this.updateFileListStateListener);
                    mMediaManager.addMediaUpdatedVideoPlaybackStateListener(this.updatedVideoPlaybackStateListener);
                    if (isMavicAir2() || isM300()) {
                        FlyDJI.getCameraInstance().enterPlayback(djiError -> {
                            if (djiError == null) {
                        ...
                        FlyDJI.getCameraInstance().setMode(SettingsDefinitions.CameraMode.MEDIA_DOWNLOAD, error -> {
                            if (error == null) {
                                DJILog.e(TAG, "Set cameraMode success");
                                showProgressDialog();
                                getFileList();
            ...
            } else if (null != FlyDJI.getCameraInstance()
                    && !FlyDJI.getCameraInstance().isMediaDownloadModeSupported()) {
    ...
    private boolean isMavicAir2() {
        BaseProduct baseProduct = FlyDJI.getProductInstance();
        if (baseProduct != null) {
    ...
    private void setResultToToast(final String string){
        PlaybackActivity.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(PlaybackActivity.this, string, Toast.LENGTH_SHORT).show();
            }
        });
    }
